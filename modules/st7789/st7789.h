...
extern mp_obj_t st7789_ST7789_write_raw(st7789_ST7789_obj_t *self, mp_obj_module_t *font, 
     const uint8_t* str_data, size_t str_len, 
     mp_int_t x, mp_int_t y, 
     mp_int_t fg_color, mp_int_t bg_color, 
     uint16_t background_width, uint16_t background_height, 
     uint16_t* background_data, bool fill);

extern void set_window(st7789_ST7789_obj_t *self, uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1);

extern void write_spi(mp_obj_base_t *spi_obj, const uint8_t *buf, int len);
