		...
		if (!strcmp(command, "t")) {
			...
			if (!do_more_prompts)
				...
				for (;;) {
					...
					zm_printf("%.*s", (int) (next_page - current_page), current_page);
					current_page = next_page;
					if (!*current_page)
						break;
					zm_printf("HELP: Type <return> for more, or q <return> to stop: ");
					fflush(stdout);
					dumb_getline(s);
					if (!strcmp(s, "q\n"))
						break;
...
char *os_read_file_name (const char *default_name, int flag)
{
	...
	if (f_setup.restore_mode) {
		...
		if (f_setup.restricted_path == NULL)
			...
			buf = zm_basename((char *)default_name);
	} else {
		if (f_setup.restricted_path != NULL) {
			sprintf(prompt, "Please enter a filename [%s]: ", zm_basename((char *)default_name));
		} else
