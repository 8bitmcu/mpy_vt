	...
	if (fg != lastfg || bg != lastbg) {
		...
		if (fg != DEFAULT_DUMB_COLOUR) {
			if (bg != DEFAULT_DUMB_COLOUR)
				zm_printf("\003%u,%s%u", (unsigned char) fg,
					((unsigned char) bg < 10) ? "0" : "", (unsigned char) bg);
			else
				zm_printf("\003%s%u", ((unsigned char) fg < 10) ? "0" : "", (unsigned char) fg);
		}
	}
...
static void show_cell_ansi(cell_t cel)
{
	...
	if (fg != lastfg) {
		if (fg < 8)
			zm_printf("\033[%dm", 30 + fg);
		else
			zm_printf("\033[38;5;%dm", 232 + fg);
		lastfg = fg;
	}

	if (bg != lastbg) {
		if (bg < 8)
			zm_printf("\033[%dm", 40 + bg);
		else
			zm_printf("\033[48;5;%dm", 232 + bg);
		lastbg = bg;
	}
...
static void show_cell_bbcode(cell_t cel)
{
	...
	if (fix != lastfix) {
		zm_printf("[font=monospace]");
		lastfix = 1;
	}

	if (bg != lastbg) {
		zm_printf("[bgcolor=%s]", bbcode_colour[(short)bg]);
		lastbg = bg;
	}

	if (fg != lastfg) {
		zm_printf("[color=%s]", bbcode_colour[(short)fg]);
		lastfg = fg;
	}
